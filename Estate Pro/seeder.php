...
// Try to locate wp-load.php by traversing upward (safe for different installs)
$max_levels = 6;
$dir = __DIR__;
$wp_load = '';
...
if ( ! $wp_load ) {
    // fallback: try relative common path (theme -> wp-content -> ../wp-load.php)
...
// 2. БЕЗПЕКА (Захист від дурня)
// Дозволяємо запуск тільки якщо це консоль (CLI) АБО якщо користувач залогінений як адмін у браузері
...
// Лічильник успішно створених об'єктів
$inserted_count = 0;
...
for ( $i = 1; $i <= 20; $i++ ) {
    ...
    if ( $post_id && ! is_wp_error( $post_id ) ) {
        // These calls use ACF; if not present they will fail silently
